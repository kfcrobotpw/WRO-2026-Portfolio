...
export const HeroRobotVisual: React.FC<HeroRobotVisualProps> = ({ onOpenPhotoManager }) => {
  const { heroData, isAdmin } = usePortfolio();
  const [isPlaying, setIsPlaying] = useState(true);
  const [activeViewMode, setActiveViewMode] = useState<'visual' | 'photo'>(
    heroData.imageUrl ? (heroData.imageMode === 'visual' ? 'visual' : 'photo') : 'visual'
  );
  ...
  // Keep view mode in sync if heroData changes
  useEffect(() => {
    if (heroData.imageMode === 'photo' && heroData.imageUrl) {
      setActiveViewMode('photo');
    } else if (heroData.imageMode === 'visual') {
      setActiveViewMode('visual');
    }
  }, [heroData.imageMode, heroData.imageUrl]);

  const filterStyle = () => {
    const filter = heroData.imageFilter;
    ...
    if (filter?.grayscale) {
      filterStr += ' grayscale(80%) hue-rotate(180deg)';
    }
    return {
      filter: filterStr,
      boxShadow: filter?.glow ? '0 0 35px rgba(34,211,238,0.5)' : undefined
    };
  };
  ...
  return (
    <div className="relative w-full rounded-2xl border border-cyan-500/30 bg-[#070e1a]/80 backdrop-blur-md overflow-hidden shadow-[0_0_30px_rgba(6,182,212,0.15)] group">
      ...
      {/* Top Cyber Frame Bar */}
      <div className="flex flex-wrap items-center justify-between px-4 py-2.5 border-b border-cyan-900/40 bg-slate-950/80 font-mono text-[11px] gap-2">
        <div className="flex items-center gap-2 text-cyan-400">
          <span className="inline-block w-2 h-2 rounded-full bg-emerald-400 animate-pulse shadow-[0_0_8px_#34d399]" />
          <span className="font-semibold tracking-wider font-['Orbitron']">
            {activeViewMode === 'photo' && heroData.imageUrl ? 'MISSION ROBOT HARDWARE' : 'PROJECT CHIMERA: PCB ASSEMBLY'}
          </span>
        </div>

        <div className="flex items-center gap-2 text-slate-400">
          {/* Mode Switcher if Photo Exists */}
          {heroData.imageUrl && (
            <div className="flex rounded-lg bg-[#050c17] p-0.5 border border-cyan-900/60 text-[10px]">
              <button
                type="button"
              ...
              <button
                ...
                title="Show cyber simulator visual"
              >
                SIMULATOR
              </button>
            </div>
          ...
          {/* Admin Instant Photo Edit Button */}
          {isAdmin && onOpenPhotoManager && (
            <button
              onClick={onOpenPhotoManager}
              className="px-2 py-1 rounded bg-cyan-950/80 border border-cyan-400 text-cyan-300 hover:bg-cyan-900 hover:text-white flex items-center gap-1 text-[10px] cursor-pointer transition-colors shadow-[0_0_10px_rgba(34,211,238,0.3)]"
              title="사진/비주얼 편집"
            >
              <Camera size={12} />
              <span>사진 편집</span>
      ...
      {/* Main Visual Display: Cyber Simulator OR Real Uploaded Photo */}
      <div className="relative h-64 sm:h-76 md:h-84 w-full bg-gradient-to-b from-[#050b14] via-[#081220] to-[#040810] flex items-center justify-center overflow-hidden">
        ...
        {/* VIEW 1: CUSTOM PHOTO (IF SELECTED) */}
        {activeViewMode === 'photo' && heroData.imageUrl ? (
          <div className="relative w-full h-full p-4 flex items-center justify-center">
            
            {/* Image Container with Cyber Frame */}
            <div className="relative w-full h-full rounded-xl overflow-hidden border border-cyan-500/50 bg-black/60 flex items-center justify-center group/img">
              <img
              ...
              <button
                ...
                className="absolute bottom-3 right-3 p-1.5 rounded-lg bg-black/80 border border-cyan-400/80 text-cyan-300 hover:bg-cyan-950 opacity-0 group-hover/img:opacity-100 transition-opacity cursor-pointer"
                title="사진 원본 크게 보기"
              >
                <Maximize2 size={14} />
              </button>
              ...
              {/* Tag Label */}
              <div className="absolute top-3 left-3 px-2 py-0.5 rounded bg-black/80 border border-cyan-500/60 font-mono text-[9px] text-cyan-300">
                OPTICAL FEED ONLINE
              </div>
            </div>
          ...
          /* VIEW 2: SVG ROBOTIC ARM & CIRCUIT SIMULATOR */
          <svg
            viewBox="0 0 500 320"
      ...
      {/* Lightbox Zoom Modal */}
      {showZoomModal && heroData.imageUrl && (
        <div 
